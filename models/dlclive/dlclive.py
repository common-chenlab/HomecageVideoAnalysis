...
class DLCLive(object):
    ...
    def get_pose(self, batch=None, **kwargs):
        """
        ...
        num_frames, height, width, _ = batch.shape
        ...
        for i in range(num_frames):
            frame = self.process_frame(batch[i,:,:,:])
        ...
        if self.model_type in ["base", "tensorrt"]:
            ...
            pose_output = self.sess.run(
                self.outputs, feed_dict={self.inputs: processed_batch}
            )
